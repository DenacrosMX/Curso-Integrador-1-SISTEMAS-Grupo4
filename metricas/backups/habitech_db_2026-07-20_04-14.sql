...
\restrict xs01dgiyK012fPcBM8RLhKWM8kj2qa7BR1OXX00aN6Sow3SHhxjqkou0vyEP9Tf
...
CREATE TABLE public.asignaciones (
    ...
    fecha_salida date,
    CONSTRAINT asignaciones_estado_check CHECK (((estado)::text = ANY ((ARRAY['ACTIVO'::character varying, 'FINALIZADO'::character varying])::text[]))),
    CONSTRAINT asignaciones_precio_mensual_pactado_check CHECK ((precio_mensual_pactado >= (0)::numeric)),
    CONSTRAINT asignaciones_tipo_adquisicion_check CHECK (((tipo_adquisicion)::text = ANY ((ARRAY['PROPIETARIO'::character varying, 'INQUILINO'::character varying, 'RESERVA'::character varying])::text[])))
);
...
CREATE TABLE public.comunicados (
    ...
    CONSTRAINT comunicados_alcance_check CHECK (((alcance)::text = ANY ((ARRAY['GLOBAL'::character varying, 'TORRE_ESPECIFICA'::character varying])::text[]))),
    CONSTRAINT comunicados_categoria_check CHECK (((categoria)::text = ANY ((ARRAY['INFORMATIVO'::character varying, 'URGENTE'::character varying, 'MANTENIMIENTO'::character varying, 'ASAMBLEA'::character varying])::text[]))),
    CONSTRAINT comunicados_estado_check CHECK (((estado)::text = ANY ((ARRAY['PUBLICADO'::character varying, 'OCULTO'::character varying])::text[])))
);
...
CREATE TABLE public.configuracion_maestra (
    ...
    CONSTRAINT configuracion_maestra_estado_check CHECK (((estado)::text = ANY ((ARRAY['ACTIVO'::character varying, 'INACTIVO'::character varying])::text[])))
);
...
CREATE TABLE public.incidencias (
    ...
    CONSTRAINT incidencias_estado_check CHECK (((estado)::text = ANY ((ARRAY['ABIERTO'::character varying, 'EN_PROCESO'::character varying, 'RESUELTO'::character varying, 'ANULADO'::character varying])::text[]))),
    CONSTRAINT incidencias_prioridad_check CHECK (((prioridad)::text = ANY ((ARRAY['BAJA'::character varying, 'MEDIA'::character varying, 'ALTA'::character varying])::text[])))
);
...
CREATE TABLE public.inventario_maestro_infraestructura (
    ...
    estado character varying(20) DEFAULT 'ACTIVO'::character varying NOT NULL,
    CONSTRAINT inventario_maestro_infraestructura_estado_check CHECK (((estado)::text = ANY ((ARRAY['ACTIVO'::character varying, 'INACTIVO'::character varying])::text[]))),
    CONSTRAINT inventario_maestro_infraestructura_tipo_elemento_check CHECK (((tipo_elemento)::text = ANY ((ARRAY['DEPARTAMENTO'::character varying, 'COCHERA'::character varying, 'SALON_RECEPCION'::character varying, 'GIMNASIO'::character varying, 'PISCINA'::character varying, 'DEPOSITO'::character varying])::text[])))
);
...
CREATE TABLE public.recibos (
    ...
    fecha_pago date,
    CONSTRAINT recibos_estado_pago_check CHECK (((estado_pago)::text = ANY ((ARRAY['PENDIENTE'::character varying, 'VALIDANDO'::character varying, 'PAGADO'::character varying, 'ANULADO'::character varying])::text[]))),
...
CREATE TABLE public.reservas (
    ...
    CONSTRAINT reservas_estado_check CHECK (((estado)::text = ANY ((ARRAY['APROBADA'::character varying, 'CANCELADA'::character varying])::text[]))),
    CONSTRAINT reservas_turno_check CHECK (((turno)::text = ANY ((ARRAY['MAÑANA'::character varying, 'TARDE'::character varying, 'NOCHE'::character varying])::text[])))
);
...
CREATE TABLE public.usuarios (
    ...
    CONSTRAINT usuarios_rol_check CHECK (((rol)::text = ANY ((ARRAY['ADMIN_SISTEMA'::character varying, 'CONSERJE'::character varying, 'RESIDENTE'::character varying])::text[])))
);
...
CREATE TABLE public.visitas (
    ...
    CONSTRAINT visitas_estado_check CHECK (((estado)::text = ANY ((ARRAY['EN_CURSO'::character varying, 'FINALIZADO'::character varying, 'ANULADO'::character varying])::text[]))),
    CONSTRAINT visitas_tipo_ingreso_check CHECK (((tipo_ingreso)::text = ANY ((ARRAY['VISITA'::character varying, 'DELIVERY'::character varying, 'SERVICIO_TECNICO'::character varying])::text[])))
);
...
1	1	Mantenimiento Mensual Estándar Base	3000.00
2	2	Mantenimiento Mensual Estándar Base	2000.00
\.
...
1	5	5	2026-07-22	MAÑANA	APROBADA	2026-07-20 09:02:46.140771+00
...
\unrestrict xs01dgiyK012fPcBM8RLhKWM8kj2qa7BR1OXX00aN6Sow3SHhxjqkou0vyEP9Tf
